...
The estimated salary variable indicates the income of each customer declared as a salary. Salaries can never be 0 or negative. Let us quickly check if that indeed is the case.

```{r}
fivenum(df_bank$EstimatedSalary)
```
Based on the five number summary of the estimated salary data, the salaries are __indeed positive which is what is expected.__
:::
